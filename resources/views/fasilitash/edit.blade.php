    ...
    <div class="row">
        <div class="col-8">
            <div class="card shadow-lg rounded-5">
                <div class="card-body">
                    <h4 class="card-title">Form Edit Fasilitas Kamar</h4>
                    <p class="card-description"> Isi data Fasilitas kamar di bawah ini dengan lengkap! </p>
                    <form action="/fasilitashotel" class="forms-sample" method="post" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="photoLama" value="{{ $fasilitas->photo }}">
                        @csrf
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control @error('nama_fasilitas') is-invalid @enderror"
                                name="nama_fasilitas" id="nama_fasilitas" placeholder="Nama_fasilitas lengkap ...."
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="status" id="status" class="form-control @error('status') is-invalid @enderror">
                                ...
                                <option value="aktif" {{ old('status') == 'aktif' || $fasilitas->status == 'aktif' ? 'selected' : '' }}>
                                    aktif</option>
                                <option value="nonaktif"{{ old('status') == 'nonaktif' || $fasilitas->status == 'nonaktif' ? 'selected' : '' }}>
                                    nonaktif</option>
                                @error('status')
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea class="form-control @error('keterangan') is-invalid @enderror" placeholder="Edit Keterangan disini..."
                                id="keterangan" name="keterangan" style="height: 120px">{{ old('keterangan') ?? $fasilitas->keterangan }}</textarea>
                            @error('keterangan')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="row my-4 align-items-center">
                            <div class="col-3">
                                <img src="{{ asset('storage/fasilitas1/' . $fasilitas->photo) }}" alt=""
